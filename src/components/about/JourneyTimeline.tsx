...
import { defaultJourney } from "@/lib/cms/defaults";
import { getIcon } from "@/lib/icons";

export function JourneyTimeline({ journey = defaultJourney }: { journey?: JourneyEntry[] }) {
  return (
    <motion.div
            ...
            return (
              ...
              >
                <div className="relative z-10 flex shrink-0 flex-col items-center">
                  <motion.div
                    className={`flex h-6 w-6 items-center justify-center rounded-full sm:h-7 sm:w-7 ${entry.node_color} shadow-lg`}
                    animate={{
                      boxShadow: [
                ...
                <div className="group min-w-0 flex-1 rounded-xl border border-white/[0.05] bg-white/[0.02] p-4 transition-colors duration-300 hover:border-white/10 hover:bg-white/[0.04] sm:p-5">
                  <div className="mb-1 flex items-start justify-between gap-3">
                    <div>
                      ...
                      <h4 className="mt-0.5 text-sm font-bold text-white sm:text-base">{entry.title}</h4>
                    </div>
                  ...
                  <p className="text-xs leading-relaxed text-white/45 sm:text-sm">{entry.description}</p>
                </div>
              </motion.div>
            );
          })}
